...
import artifact from "../backend/build/contracts/BlindAuction.json";
import {
  ENVIRONMENT,
  infuraWSSRopsten,
  infuraWSSGoerli,
...
const web3 =
  ENVIRONMENT.toUpperCase() === "GANACHE"
    ...
    ? new Web3(
        Web3.currentProvider ||
          new Web3.providers.WebsocketProvider(infuraWSSRopsten)
      )
    : ENVIRONMENT.toUpperCase() === "GOERLI"
    ? new Web3(
        Web3.currentProvider ||
          new Web3.providers.WebsocketProvider(infuraWSSGoerli)
      )
    : "";
...
export const biddingEnd = async (contractAddress) => {
  ...
  let result = BigNumber(
    await contract.methods.biddingEnd().call({ from: myAddress })
  );
...
export const revealEnd = async (contractAddress) => {
  const contract = new web3.eth.Contract(artifact.abi, contractAddress);
  let result = BigNumber(
    await contract.methods.revealEnd().call({ from: myAddress })
  );
  let output = result.c[0];
...
export const highestBidder = async (contractAddress) => {
  ...
  const contract = new web3.eth.Contract(artifact.abi, contractAddress);
  let highestBidderAddreses = await contract.methods
    .highestBidder()
    .call({ from: myAddress });
...
export const highestBid = async (contractAddress) => {
  const contract = new web3.eth.Contract(artifact.abi, contractAddress);
  let highestBidValue = await contract.methods
    .highestBid()
    .call({ from: myAddress });
