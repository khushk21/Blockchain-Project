...
import { checkExpired, getAuctionURL } from "../controller/dns_backend.js";
...
class DomainStatus_ extends React.Component {
  ...
  componentDidMount() {
    const checkStatus = async (queryDomainName) => {
      ...
      if (
        result.auctionAddress === "0x0000000000000000000000000000000000000000"
      ) {
        this.setState({
  ...
  render() {
    ...
    switch (this.state.status) {
      ...
      case "EXPIRED_NO_AUCTION":
        status = <ExpiredNoAuction domainName={this.state.domainName} />;
        break;
